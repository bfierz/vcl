...
namespace Vcl { namespace Physics { namespace Fluid { namespace Cuda
{
	...
	)
	{
		...
		Require(dynamic_pointer_cast<Buffer>(obstacles), "obstacles is CUDA buffer");
		Require(dynamic_pointer_cast<Buffer>(pressure), "pressure is CUDA buffer");
		Require(dynamic_pointer_cast<Buffer>(divergence), "divergence is CUDA buffer");
	...
	void CenterGrid3DPoissonJacobiCtx::resize(int size)
	{
		using Compute::Cuda::Buffer;

		Require(this->size() == size, "Size matches");
