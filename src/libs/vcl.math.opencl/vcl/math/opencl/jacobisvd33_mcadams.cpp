...
namespace Vcl { namespace Mathematics { namespace OpenCL {
		...
		Vcl::Core::InterleavedArray<float, 3, 1, -1>& outS)
	{
		...
		static_cast<Compute::OpenCL::Kernel*>(_svdKernel.get())->run(static_cast<Compute::OpenCL::CommandQueue&>(queue), 1, grid, block, (int)size, (int)_capacity, (cl_mem)*A, (cl_mem)*U, (cl_mem)*V, (cl_mem)*S);
