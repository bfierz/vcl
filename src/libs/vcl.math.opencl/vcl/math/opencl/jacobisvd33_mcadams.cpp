...
// Kernels
extern uint32_t JacobiSVD33McAdamsKernelCL[];
...
namespace Vcl { namespace Mathematics { namespace OpenCL
{
	...
	: _ownerCtx(ctx)
	{
		...
		// Load the module
		_svdModule = ctx->createModuleFromSource(reinterpret_cast<const char*>(JacobiSVD33McAdamsKernelCL));
	...
	)
	{
		...
		auto& queue = _ownerCtx->defaultQueue();
